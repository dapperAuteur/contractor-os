...
export async function seedLister(db: SupabaseClient, userId: string): Promise<void> {
  ...
  const { error: txErr } = await db.from('financial_transactions').insert([
    ...
    { user_id: userId, amount: 49.99, type: 'expense', description: 'CrewOps monthly subscription', vendor: 'Work.WitUS', transaction_date: daysAgo(15), source: 'manual', account_id: bizCreditId },
