# Shared Database: JobHub + CentenarianOS

Both **JobHub** (contractor-os) and **CentenarianOS** (centenarian-os) share the same Supabase database and `profiles` table. Migrations in either repo must not break the other app.
...
## Columns added by JobHub (not used by CentenarianOS)
...
## Columns added by CentenarianOS (not used by JobHub)

_None currently tracked. Add entries here when CentOS adds columns that JobHub doesn't use._
