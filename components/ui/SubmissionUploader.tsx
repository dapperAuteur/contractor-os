...
function FilePreview({ file, onRemove }: { file: SubmissionFile; onRemove: () => void }) {
  ...
  return (
    <div className="flex items-start gap-3 bg-slate-100 rounded-xl p-3">
      {isImg ? (
      ...
      <div className="flex-1 min-w-0">
        <p className="text-sm text-slate-800 truncate mb-1">{file.name}</p>
        {isVid && (
      ...
      <button
        type="button"
        onClick={onRemove}
        className="p-1 text-slate-400 hover:text-red-400 transition shrink-0 mt-0.5"
      >
        <X className="w-4 h-4" />
      </button>
    </div>
      ...
      {files.length < maxFiles && (
        <>
          <button
            ...
            className="flex items-center gap-2 px-3 py-2 rounded-lg text-xs font-medium bg-slate-100 text-slate-700 hover:bg-slate-200 transition disabled:opacity-50"
          >
            {uploading
