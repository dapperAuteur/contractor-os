  ...
  return (
    <div className={className}>
      ...
      <button
        onClick={handleClick}
        disabled={loading}
        className="inline-flex items-center justify-center px-6 py-3 bg-amber-600 text-white rounded-lg hover:bg-amber-500 transition font-semibold disabled:opacity-60 disabled:cursor-not-allowed min-h-11"
      >
        {loading ? (
          <span className="animate-spin inline-block w-4 h-4 border-2 border-amber-200 border-t-transparent rounded-full mr-2" />
        ) : (
          <Play className="w-4 h-4 mr-2" />
        )}
        {loading ? 'Logging in...' : label ?? 'Try the Demo'}
      </button>
      <p className="text-neutral-400 text-xs mt-2 flex items-center justify-center gap-1">
