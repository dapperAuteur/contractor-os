...
interface Props {
  ...
  onChange: (ex: LogExercise) => void;
  onRemove: () => void;
  equipment?: { id: string; name: string }[];
}

export default function LogExerciseRow({ exercise, onChange, onRemove, equipment }: Props) {
  const [showAdvanced, setShowAdvanced] = useState(false);

  const set = <K extends keyof LogExercise>(key: K, val: LogExercise[K]) => {
    onChange({ ...exercise, [key]: val });
  };

  const toggleBool = (key: keyof LogExercise) => {
    onChange({ ...exercise, [key]: !exercise[key] });
  };

  const boolFlags: { key: keyof LogExercise; label: string }[] = [
    ...
    <div className="border border-gray-200 rounded-lg p-3 space-y-3" role="group" aria-label={exercise.name || 'Exercise'}>
      ...
      <div className="flex items-start gap-2">
        ...
        <div className="flex-1 grid grid-cols-2 sm:grid-cols-6 gap-2">
          <div className="col-span-2">
            <ExercisePicker
              ...
              onChange={(name, exerciseId, defaults) => {
                const updates: Partial<LogExercise> = { name, exercise_id: exerciseId };
                if (defaults) {
                  if (defaults.sets != null) updates.sets_completed = defaults.sets;
                  if (defaults.reps != null) updates.reps_completed = defaults.reps;
          ...
          <input type="number" placeholder="Sets" value={exercise.sets_completed ?? ''}
            onChange={(e) => set('sets_completed', e.target.value === '' ? null : parseInt(e.target.value))}
            aria-label="Sets"
          ...
          {exercise.is_timed ? (
            ...
            <input type="number" placeholder={exercise.per_side ? 'Reps/side' : 'Reps'} value={exercise.reps_completed ?? ''}
              onChange={(e) => set('reps_completed', e.target.value === '' ? null : parseInt(e.target.value))}
      ...
      {showAdvanced && (
        <div className="pl-6 space-y-3 border-t border-gray-100 pt-3">
          ...
          {/* Side + Feeling (log-only) */}
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
            {exercise.is_unilateral && (
              <div>
                <label className="block text-xs text-gray-500 mb-0.5">Side</label>
                <select value={exercise.side || ''}
            ...
            <div>
              <label className="block text-xs text-gray-500 mb-0.5">Feeling</label>
              <select value={exercise.feeling ?? ''}
                onChange={(e) => set('feeling', e.target.value === '' ? null : parseInt(e.target.value))}
                className="w-full px-2 py-1.5 border border-gray-300 rounded text-sm bg-white">
                <option value="">—</option>
                {[1, 2, 3, 4, 5].map((v) => (
                  <option key={v} value={v}>{v} — {FEELING_LABELS[v]}</option>
                ))}
              </select>
            </div>
          </div>
