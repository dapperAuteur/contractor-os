      ...
      {loading ? (
        ...
        <div className="space-y-2" role="list" aria-label="Job offers">
          {filtered.map((a) => {
            ...
            return (
              <article key={a.id} role="listitem" className={`rounded-xl border bg-white p-4 ${isOffer ? 'border-yellow-700/50' : 'border-slate-200'}`}>
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0 flex-1">
                    ...
                    {a.message && (
                      <p className="mt-2 text-sm text-slate-700 bg-slate-100/50 rounded-lg px-3 py-2 italic">
                        &ldquo;{a.message}&rdquo;
                      </p>
                    )}
                    {a.response_note && (
                      <p className="mt-1 text-xs text-slate-500">
                        <span className="text-slate-400">Your response:</span> {a.response_note}
                      </p>
                    )}
                  </div>
                </div>
                  ...
                  <div className="mt-3 space-y-2">
                    ...
                    <div className="flex gap-2 flex-wrap">
                      <button
                        ...
                        className="flex items-center gap-1.5 rounded-lg bg-green-600 px-4 py-2.5 text-sm font-medium text-white hover:bg-green-500 disabled:opacity-50 focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2 focus:ring-offset-white"
                      >
                      ...
                      <button
                        ...
                        disabled={respondingId === a.id}
                        className="flex items-center gap-1.5 rounded-lg bg-red-600 px-4 py-2.5 text-sm font-medium text-white hover:bg-red-500 disabled:opacity-50 focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2 focus:ring-offset-white"
                      >
                      ...
                      <button
                        ...
                        className="flex items-center gap-1.5 rounded-lg border border-slate-300 px-3 py-2.5 text-sm text-slate-500 hover:text-slate-800 hover:bg-slate-100 focus:outline-none focus:ring-2 focus:ring-amber-500"
                      >
