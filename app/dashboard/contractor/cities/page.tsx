  ...
  return (
    <div className="mx-auto max-w-4xl space-y-4 p-4">
      <div className="flex items-center justify-between gap-3 flex-wrap">
        <h1 className="text-2xl font-bold text-slate-900">City Guides</h1>
        <button
      ...
      {/* Tabs */}
      <div className="flex gap-1 border-b border-slate-200" role="tablist" aria-label="Guide views">
        {[
      ...
      {showCreate && (
        <div className="rounded-xl border border-slate-300 bg-white p-4 space-y-3">
          <h2 className="text-lg font-semibold text-slate-900">New City Guide</h2>
        ...
        <div className="space-y-2" role="list" aria-label={tab === 'mine' ? 'My city guides' : 'Shared city guides'}>
          {filtered.map((guide) => (
            <button
              ...
              className="flex min-h-11 w-full items-center justify-between rounded-xl border border-slate-200 bg-white p-4 text-left transition-colors hover:border-slate-300 focus:outline-none focus:ring-2 focus:ring-amber-500"
            ...
            >
              <div className="min-w-0 flex-1">
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  <span className="font-medium text-slate-900">{guide.city_name}</span>
                  {guide.state && <span className="text-sm text-slate-500">{guide.state}</span>}
                  {guide.region && <span className="text-xs text-slate-400">· {guide.region}</span>}
                  {guide.is_shared ? (
                    <Globe size={12} className="text-green-400" aria-label="Shared" />
                  ) : (
                    <Lock size={12} className="text-slate-400" aria-label="Private" />
                  )}
                </div>
