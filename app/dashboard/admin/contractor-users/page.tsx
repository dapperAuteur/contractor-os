    ...
    <div className="mx-auto max-w-6xl space-y-4 p-4">
      ...
      {loading ? (
        <div className="flex justify-center py-16">
          <Loader2 className="animate-spin text-slate-400" size={28} aria-label="Loading users" />
        </div>
      ) : filtered.length === 0 ? (
        ...
        <div className="space-y-2" role="list" aria-label="User management list">
          {filtered.map((u) => {
            ...
            return (
              <article key={u.id} role="listitem" className={`rounded-xl border bg-white p-4 ${hasEdits ? 'border-amber-700/50' : 'border-slate-200'}`}>
                <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                  {/* User info */}
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2 flex-wrap">
                      <UserCircle size={16} className="text-slate-400 shrink-0" aria-hidden="true" />
                      <span className="font-medium text-slate-900 text-sm">{u.username || 'No username'}</span>
                      ...
                      {currentProducts.map((p) => (
                        <span key={p} className="rounded-full bg-slate-100 px-2 py-0.5 text-xs text-slate-500">
                          {PRODUCT_LABELS[p] ?? p}
                        </span>
                      ))}
                    </div>
                  ...
                  <div className="flex items-center gap-2 flex-wrap">
                    <label className="flex items-center gap-1.5">
                      ...
                      <select
                        ...
                        className="rounded border border-slate-200 bg-slate-100 px-2 py-1.5 text-xs text-slate-900 focus:border-amber-500 focus:outline-none min-h-11"
                      ...
                      {(['centos', 'contractor', 'lister'] as const).map((product) => (
                        <label key={product} className="flex items-center gap-1 cursor-pointer">
                          <input
                            ...
                            className="rounded border-neutral-600 bg-slate-100 text-amber-500 focus:ring-amber-500 h-4 w-4"
                            aria-label={`${PRODUCT_LABELS[product]} access for ${u.username || u.email}`}
                          />
                          <span className="text-xs text-slate-500">{product === 'centos' ? 'OS' : product === 'contractor' ? 'Cont' : 'List'}</span>
                        </label>
                      ))}
                    </div>
